...
from arcadia.building.images.image_builder_windows_pipeline_nestedstack import (
    ImageBuilderWindowsPipeline,
)
    ...
    def __init__(
        ...
        **kwargs
        ...
        assets_stack = BuildAssetsNestedStack(
            self,
            "BuildAssetsNestedStack",
            bucket=bucket,
            components_prefix=components_prefix,
        )

        instance_profile = AuthorizationNestedStack(
            self, "AuthorizationNestedStack", removal_policy=removal_policy
        ).instance_profile

        distribution_configuration_stack = Distribution(
            self,
            "Distribution",
            removal_policy=removal_policy,
            version=version,
            organization_arns=self.node.try_get_context("organization_arns"),
            target_account_ids=self.node.try_get_context("target_account_ids"),
        )

        pipeline = ImageBuilderWindowsPipeline(
            self,
            "ImageBuilderWindowsPipeline",
            ...
            distribution_configuration=distribution_configuration_stack.distro,
        )
        pipeline.add_dependency(assets_stack)
        pipeline.add_dependency(distribution_configuration_stack)
